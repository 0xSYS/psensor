...
static void populate(struct ui_psensor *ui)
{
	...
	for(s_cur = ordered_sensors; *s_cur; s_cur++)
	{
		...
		gtk_list_store_set(store, &iter, COL_NAME, s->name, COL_COLOR_STR, scolor, COL_GRAPH_ENABLED, config_is_sensor_graph_enabled(s->id), COL_SENSOR, s, COL_DISPLAY_ENABLED, enabled, -1);
		free(scolor);
		gdk_rgba_free(color);
	}
...
void ui_sensorlist_update(struct ui_psensor *ui, bool complete)
{
	...
	while(valid)
	{
		...
		value = psensor_value_to_str(s->type, psensor_get_current_value(s), use_celsius);
		min = psensor_value_to_str(s->type, s->sess_lowest, use_celsius);
...
static void hide_activated_cbk(GtkWidget *menu_item, gpointer data)
{
	...
	while(valid)
	{
		...
		if(s == s2)
			gtk_list_store_set(cb_data->ui->sensors_store, &iter, COL_DISPLAY_ENABLED, false, -1);
		valid = gtk_tree_model_iter_next(model, &iter);
	}
...
static GtkWidget *create_sensor_popup(struct ui_psensor *ui, struct psensor *sensor)
{
	...
	data->ui = ui;
	data->sensor = sensor;
	g_signal_connect(item, "activate", G_CALLBACK(hide_activated_cbk), data);
	...
	data->ui = ui;
	data->sensor = sensor;
	g_signal_connect(item, "activate", G_CALLBACK(preferences_activated_cbk), data);
...
static int clicked_cbk(GtkWidget *widget, GdkEventButton *event, gpointer data)
{
	...
	if(s)
	{
		...
		{
			color = config_get_sensor_color(s->id);
			if(ui_change_color(_("Select sensor color"), color, GTK_WINDOW(ui->main_window)))
			{
...
void ui_sensorlist_cb_graph_toggled(GtkCellRendererToggle *cell, gchar *path_str, gpointer data)
{
	struct ui_psensor *ui;
	GtkTreeModel *model, *fmodel;
	GtkTreeIter iter;
	...
	while(valid)
	{
		...
		if(s == s2)
			gtk_list_store_set(ui->sensors_store, &iter, COL_GRAPH_ENABLED, b, -1);
		valid = gtk_tree_model_iter_next(model, &iter);
	}
}
...
void ui_sensorlist_create(struct ui_psensor *ui)
{
	...
	gtk_tree_model_filter_set_visible_column(GTK_TREE_MODEL_FILTER(fmodel), COL_DISPLAY_ENABLED);
	gtk_tree_view_set_model(ui->sensors_tree, fmodel);
	g_signal_connect(ui->sensors_tree, "button-press-event", (GCallback)clicked_cbk, ui);
	ui_sensorlist_update(ui, 1);
	log_fct_exit();
}