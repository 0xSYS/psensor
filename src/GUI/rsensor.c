...
struct psensor **get_remote_sensors(const char *server_url, int values_max_length)
{
	...
	if(obj && !is_error(obj))
	{
		...
		for(i = 0; i < n; i++)
		{
			s = psensor_new_from_json(json_object_array_get_idx(obj, i), url, values_max_length);
			sensors[i] = s;
		}
...
static void remote_psensor_update(struct psensor *s)
{
	...
	if(obj && !is_error(obj))
	{
		...
		if(!is_error(obj))
		{
			...
			psensor_set_current_measure(s, json_object_get_double(ov), tv);
		}
