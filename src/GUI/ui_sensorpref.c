...
static void update_pref(struct psensor *s)
{
	...
	gtk_toggle_button_set_active(w_sensor_draw, config_is_sensor_graph_enabled(s->id));

	gtk_toggle_button_set_active(w_sensor_display, config_is_sensor_enabled(s->id));

	color = config_get_sensor_color(s->id);
	gtk_color_chooser_set_rgba(GTK_COLOR_CHOOSER(w_sensor_color), color);
	gdk_rgba_free(color);

	gtk_label_set_text(w_sensor_high_threshold_unit, psensor_type_to_unit_str(s->type, use_celsius));
	gtk_label_set_text(w_sensor_low_threshold_unit, psensor_type_to_unit_str(s->type, use_celsius));
	...
	gtk_toggle_button_set_active(w_sensor_alarm, config_get_sensor_alarm_enabled(s->id));
	...
	gtk_toggle_button_set_active(w_appindicator_enabled, config_is_appindicator_enabled(s->id));
	gtk_toggle_button_set_active(w_appindicator_label_enabled, config_is_appindicator_label_enabled(s->id));
...
static GtkBuilder *load_ui(struct ui_psensor *ui)
{
	...
	ok = gtk_builder_add_from_file(builder, "/usr/local/share/psensor/sensor-edit.glade", &error);
	...
	store = GTK_LIST_STORE(gtk_builder_get_object(builder, "sensors_liststore"));
	gtk_builder_connect_signals(builder, ui);
	g_signal_connect(w_dialog, "delete_event", G_CALLBACK(on_delete_event_cb), w_dialog);
...
static void populate(struct psensor *sensor, struct psensor **sensors)
{
	...
	for(s_cur = ordered_sensors; *s_cur; s_cur++)
	{
		...
		gtk_list_store_set(store, &iter, COL_NAME, s->name, COL_SENSOR_PREF, s, -1);
	}
