...
static bool is_bcm2835_present(void)
{
	...
	if(!ret)
	{
		log_debug("%s: %s does not exist.", PROVIDER_NAME, SYS_RPI_DETECTION_FILE);
		return false;
	}

	str = file_get_content(SYS_RPI_DETECTION_FILE);

	if(!str || strncmp(str, SYS_RPI_DETECTION_FILE_EXPECTED_CONTENT, strlen(SYS_RPI_DETECTION_FILE_EXPECTED_CONTENT) - 1))
	{
...
static double bcm2835_update_temp(struct psensor *s)
{
	...
	if(str)
	{
		l = strtol(str, &end, 10);
		if(*end != '\0')
			log_debug("%s: found invalid value: %s.", PROVIDER_NAME, str);
		else
