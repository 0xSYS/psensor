...
static struct psensor *create_sensor(char *id, char *name, SkDisk *disk, int values_max_length)
{
	struct psensor *s;
	int t;
...
void atasmart_psensor_list_append(struct psensor ***sensors, int values_max_length)
{
	char **paths, **tmp, *id;
	SkDisk *disk;
	...
	while(*tmp)
	{
		...
		if(!sk_disk_open(*tmp, &disk))
		{
			...
			sensor = create_sensor(id, *tmp, disk, values_max_length);
