...
static void lmsensor_data_set(struct psensor *s, const struct sensors_chip_name *chip, const struct sensors_feature *feature)
{
...
static struct psensor *lmsensor_psensor_create(const sensors_chip_name *chip, const sensors_feature *feature, int values_max_length)
{
	char name[200];
	const sensors_subfeature *sf;
	int type;
	...
	id = malloc(strlen(PROVIDER_NAME) + 1 + strlen(name) + 1 + strlen(label) + 1);
	...
	if(feature->type == SENSORS_FEATURE_TEMP && (get_temp_input(psensor) == UNKNOWN_DBL_VALUE))
	{
		free(psensor);
		return NULL;
	}
