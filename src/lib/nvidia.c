...
const char *PROVIDER_NAME = "nvctrl";

void set_nvidia_id(struct psensor *s, int id)
{
	*(int *)s->provider_data = id;
}

int get_nvidia_id(struct psensor *s)
{
	return *(int *)s->provider_data;
}

char *get_product_name(int id, int type)
{
	char *name;
	Bool res;
...
double get_att(int target, int id, int att)
{
	Bool res;
	int temp;
...
double get_usage_att(char *atts, const char *att)
{
	char *c, *key, *strv, *s;
	size_t n;
	double v;
...
double get_usage(int id, int type)
{
	const char *stype;
	char *atts;
	double v;
...
double get_value(int id, int type)
{
	int att;
...
void update(struct psensor *sensor)
{
	double v;
	int id;
...
int check_sensor(int id, int type)
{
...
struct psensor *create_nvidia_sensor(int id, int subtype, int value_len)
{
...
void add(struct psensor ***sensors, int id, int type, int values_len)
{
	struct psensor *s;
