...
static struct psensor *create_sensor(int id, int type, int values_len)
{
	...
	s = psensor_create(sid, strdup(name), strdup("AMD/ATI GPU"), sensor_type, values_len);
...
static int init(void)
{
	...
	adl_od5_temperature_get          = (ADL_OD5_TEMPERATURE_GET)          getprocaddress(hdll, "ADL_Overdrive5_Temperature_Get");
	adl_od5_fanspeed_get             = (ADL_OD5_FANSPEED_GET)             getprocaddress(hdll, "ADL_Overdrive5_FanSpeed_Get");
	...
	for(i = 0; i < inumberadapters; i++)
	{
		...
		else
		{
			...
			active_adapters = (int *)realloc(active_adapters, sizeof(int)*inumberadaptersactive);
