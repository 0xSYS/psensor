...
{
    memmove(s->measures, &s->measures[1], (s->values_max_length - 1) * sizeof(struct measure));
...
const char *psensor_type_to_str(unsigned int type)
{
    ...
    if(type & SENSOR_TYPE_ROC_SMI)
    {
        ...
        else if(type & SENSOR_TYPE_GPU_POWER_AVG)
            return "AMD GPU Power Avg";
    }
...
const char *psensor_type_to_unit_str(unsigned int type, int use_celsius)
{
    ...
    else if(type & SENSOR_TYPE_PWM)
    {
        return " PWM";
    }
    else if(type & SENSOR_TYPE_WATT)
    {
        return " W";
    }
...
char *psensor_current_value_to_str(const struct psensor *s, unsigned int use_celsius)
{
    return psensor_value_to_str(s->type, psensor_get_current_value(s), use_celsius);
}